...
17. [on_error 与 JumpBack 的交互及 error_handling 标志](#17-on_error-与-jumpback-的交互及-error_handling-标志)
18. [context.override_next() 动态修改 next 列表](#18-contextoverride_next-动态修改-next-列表)
19. [商店购买子任务间跳转与空壳节点死循环](#19-商店购买子任务间跳转与空壳节点死循环)
...
> ⚠️ **重要**：`success=False` 会设置 `error_handling=true`，而 JumpBack 栈弹出条件之一是 `!error_handling`。这意味着在 `on_error` 路径中，JumpBack 回退机制可能被阻断。如果需要从子任务跳回父任务入口（如游荡商人完成→商店购买_入口），应使用 `context.override_next()` + `success=True` 而非 `success=False` + `on_error`，详见 [§17](#17-on_error-与-jumpback-的交互及-error_handling-标志)。
...
## 17. on_error 与 JumpBack 的交互及 error_handling 标志
...
## 18. context.override_next() 动态修改 next 列表
