...
9. [行军识别无限等待问题与超时机制](#9-行军识别无限等待问题与超时机制)
10. [context.run_recognition 返回 None 的条件](#10-contextrun_recognition-返回-none-的条件)
11. [JumpBack 机制与嵌套节点执行顺序](#11-jumpback-机制与嵌套节点执行顺序)
12. [detail.box 与 detail.best_result.box 的关系](#12-detailbox-与-detailbest_resultbox-的关系)
13. [ColorMatch 取色识别 API](#13-colormatch-取色识别-api)
14. [Rect 偏移量计算规则](#14-rect-偏移量计算规则)
15. [override_pipeline 三层级作用域与跨任务持久化](#15-override_pipeline-三层级作用域与跨任务持久化)
16. [on_error 与 JumpBack 的交互及 error_handling 标志](#16-on_error-与-jumpback-的交互及-error_handling-标志)
17. [context.override_next() 动态修改 next 列表](#17-contextoverride_next-动态修改-next-列表)
18. [商店购买子任务间跳转与空壳节点死循环](#18-商店购买子任务间跳转与空壳节点死循环)
...
> ⚠️ **重要**：`success=False` 会设置 `error_handling=true`，而 JumpBack 栈弹出条件之一是 `!error_handling`。这意味着在 `on_error` 路径中，JumpBack 回退机制可能被阻断。如果需要从子任务跳回父任务入口（如游荡商人完成→商店购买_入口），应使用 `context.override_next()` + `success=True` 而非 `success=False` + `on_error`，详见 [§16](#16-on_error-与-jumpback-的交互及-error_handling-标志)。
...
## 10. context.run_recognition 返回 None 的条件
...
## 12. detail.box 与 detail.best_result.box 的关系
...
## 15. override_pipeline 三层级作用域与跨任务持久化
...
## 16. on_error 与 JumpBack 的交互及 error_handling 标志
...
## 17. context.override_next() 动态修改 next 列表
