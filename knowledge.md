...
#### 底层实现（基于 MaaFramework v5.10.4 PipelineTask.cpp 源码）

> ⚠️ `jump_back` 是**引用属性**（next 列表项上的 bool，由 `[JumpBack]` 前缀标记），不是节点定义属性。同一个节点在不同 next 列表中的引用可以不同（有的带前缀，有的不带）。

JumpBack 栈存储的是**父节点的名字符串**，不是 next 列表本身：

```cpp
std::stack<std::string> jumpback_stack;   // 栈元素是节点名字符串

// 命中带 [JumpBack] 前缀的引用项时
if (node_detail.jump_back) {
    jumpback_stack.emplace(pre_node_name);  // 压入"当前父节点的名字"
}

// 后续链路 next 跑空时
if (next.empty() && !error_handling && !jumpback_stack.empty()) {
    auto top = std::move(jumpback_stack.top());  // 弹出父节点名
    jumpback_stack.pop();
...
1. 命中 `[JumpBack]X` 时，压栈的是**当前正在遍历的父节点的名字**（不是 X，也不是 next 列表）
2. 子链 `next` 跑空时，弹栈得到父节点名 → 重新查其 `next` → **从父节点 next 列表开头重新遍历**
3. 同一节点以 `[JumpBack]` 引用时压栈；以裸名（无前缀）引用时**不压栈**
4. DirectHit 节点（无 `recognition` 字段）总是匹配成功，配合 JumpBack 会形成无限循环，**必须**在其前放置有条件的识别节点
...
执行顺序：
1. 遍历 `入口.next`，执行 `每日检查`（Custom Action），若 `success=True` → 继续
2. 执行 `已在界面`（OCR），若识别到 → 进入其 `next`（此时若 `已在界面.next` 跑空，弹栈回 `入口.next` 开头）
3. 若 `已在界面` 未识别到 → 继续到 `[JumpBack]点击商店`
4. `[JumpBack]点击商店` 识别成功 → **压栈 `"入口"`** → 执行其 action → 若 `点击商店.next` 跑空 → 弹栈得 `"入口"` → 重新从 `入口.next` 开头遍历
5. 第二轮若 `每日检查` 返回 `success=True`，会直接到 `已在界面`，不会再触发 `[JumpBack]点击商店`

#### 同一节点不同引用方式的行为

同一节点 `新手_等待` 被多处引用：
- 在 `新手_入口.next` 中以 `[JumpBack]新手_等待` 引用 → 命中时**压栈** `新手_入口`
- 在 `训练士兵-关闭面板.next` 中以 `新手_等待`（裸名）引用 → 命中时**不压栈**

因此从 `训练士兵-关闭面板` 进入 `新手_等待` 后，`新手_等待.next` 跑空时弹栈得到的是更早压入的 `新手_入口`，回到主循环开头。三种引用方式都能正常回主循环。
...
当节点的 `next` 列表中**所有项**都被 `enabled: false` 禁用时，**任务自然终止**，不会死循环。源码逻辑：
...
注意：这与 `next` 为**空列表**不同——`next` 为空列表时触发 JumpBack 弹栈（回父节点 next 开头），而 `next` 非空但全部禁用时触发任务终止。

#### 任务 SUCCEEDED 的条件

`PipelineTask::run()` 返回 `true`（SUCCEEDED）只有两条路径：
1. `run_next` 返回后检查 `need_to_stop()` 为 true（外部停止信号）→ `return true`
2. `while (!next.empty())` 退出且 `!error_handling`（即 next 空 + 栈空 + 无错误）

正常无限循环的任务（如新手入口含 DirectHit 兜底节点）不会自行 SUCCEEDED；若观察到 SUCCEEDED，通常是外部 `need_to_stop()` 触发。
